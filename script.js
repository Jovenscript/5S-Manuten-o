...
// Reduz para no máximo 400px / 65% de qualidade antes de salvar no Firebase.
// Uma foto de celular (~3MB) vira ~20-40KB com isso.
// =========================================================================
...
// =========================================================================
// SALVAMENTO FIREBASE — DOCUMENTOS SEPARADOS
//
// PROBLEMA ANTERIOR: tudo em 1 documento "dados_sistema" -> batia no limite
// de 1MB do Firestore porque as imagens base64 eram guardadas junto.
//
// SOLUÇÃO: dividir em 14 documentos pequenos:
//   manutencao_5s/config      -> gavetas + usuários  (~2KB)
//   manutencao_5s/historico   -> logs                (~80KB)
//   manutencao_5s/itens_g1    -> itens da gaveta 1   (~50KB máx)
//   manutencao_5s/itens_g2    -> itens da gaveta 2
//   ... etc ...
//   manutencao_5s/itens_g12   -> itens da gaveta 12
//
// Cada operação salva SOMENTE o documento que foi modificado.
// =========================================================================
...
async function salvarItensDaGaveta(idGaveta) {
    try {
        ...
        console.error(`Erro ao salvar itens da gaveta ${idGaveta}:`, e);
...
async function iniciarSincronizacaoFirebase() {
    // Migra dados do documento antigo (se existir) antes de ligar os listeners
    await migrarDadosLegados();

    // Listener: config (gavetas + usuários)
    onSnapshot(doc(db, "manutencao_5s", "config"), (snap) => {
        if (snap.exists()) {
            ...
            database.drawers.forEach(g => {
                if (!database.items[g.id]) database.items[g.id] = [];
            });
        } else {
            salvarConfig();
        }
    ...
    // Listeners: itens de cada gaveta (um documento por gaveta)
...
// Migra o documento legado "dados_sistema" para a nova estrutura separada.
// Remove as imagens do legado pois elas que causavam o estouro do limite.
async function migrarDadosLegados() {
    try {
        ...
        if (!legadoSnap.exists() || configSnap.exists()) return; // Nada a migrar

        console.log("Migrando dados do documento legado para nova estrutura...");
        ...
        for (const gaveta of GAVETAS_PADRAO) {
            const itens = ((db_legado.items || {})[gaveta.id] || []).map(peca => ({
                ...peca,
                image: null // Remove imagens do legado; recadastre-as após a migração
            }));
            await setDoc(doc(db, "manutencao_5s", `itens_g${gaveta.id}`), { items: itens });
        }

        console.log("Migração concluída. As imagens antigas foram removidas para liberar espaço — recadastre-as normalmente.");
    } catch (e) {
...
function exportarEstoqueCSV() {
    ...
    database.drawers.forEach(gaveta => {
        (database.items[gaveta.id] || []).forEach(peca => {
            ...
            csv += `"${gaveta.label} - ${gaveta.title}","${peca.code || ''}","${peca.name}",${peca.expected},${peca.current},"${status}"\n`;
        });
    });
...
function calcularKPIs() {
    ...
    database.drawers.forEach(gaveta => {
        ...
        div.innerHTML      = `<i class="fa-solid fa-circle-${icone}"></i> ${gaveta.label}: ${getStatusText(statusGaveta)}`;
...
function renderArmarioVertical() {
    ...
    chassi.innerHTML = '';
    database.drawers.forEach(gaveta => {
        const statusGaveta = getGavetaStatus(database.items[gaveta.id] || []);
...
function salvarNovoItem() {
    ...
    if (imgInput.files && imgInput.files[0]) {
        comprimirImagem(imgInput.files[0]).then(base64 => { novaPeca.image = base64; finalizar(); });
    } else {
        finalizar();
    }
}
...
function salvarEdicaoPeca() {
    ...
    if (imgInput.files && imgInput.files[0]) {
        comprimirImagem(imgInput.files[0]).then(base64 => { peca.image = base64; finalizar(); });
    } else {
        finalizar();
    }
}
...
function gerarEmailPedido() {
    ...
    if (itens.length === 0) return mostrarAlerta("Tudo em Ordem", "Não há peças faltando no gaveteiro neste momento.");
